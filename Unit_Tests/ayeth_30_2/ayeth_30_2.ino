...
String inputCValue = "", inputVValue = "", outputCValue = "";
bool enteringFirstInput = true, enteringSecondInput = false, inputCHasDecimal = false, inputVHasDecimal = false, outputCHasDecimal = false, finalOutputIsDisplayed = false;
float inputC = 0.0, inputV = 0.0, outputC = 0.0, temp = 0.0;
double outputV = 0.0;
char outputVStr[10];
...
void loop() {
  ...
  if (finalOutputIsDisplayed && !completed) {
  ...
  if (myData.ultrasonicObjectDetected && !waterFilled && !BIsPressed) {
  ...
  switch (currentState) {
    ...
    case INPUT_C_STATE:
      ...
      if (enteringFirstInput) {
        ...
        lcd.clear();
        displayInitialMessage("Input V = ", "ml");
        enteringFirstInput = false;
        enteringSecondInput = true;
        currentState = INPUT_V_STATE;
      }
      break;

    case INPUT_V_STATE:
      if (enteringSecondInput) {
        inputV = inputVValue.toFloat();
        Serial.print("Input V = ");
        Serial.print(inputV);
        Serial.println(" ml");
        lcd.clear();
        displayInitialMessage("Output V = ", "moldm");
    ...
    case OUTPUT_C_STATE:
      ...
      if (!enteringFirstInput && !enteringSecondInput) {
...
void handleAsteriskKey() {
  if (inputCValue.length() > 0 && inputVValue.length() == 0) {
    clearInputCValue();
  } else if (inputCValue.length() > 0 && inputVValue.length() > 0 && outputCValue.length() == 0) {
    clearInputVValue();
  }
  else if (inputCValue.length() > 0 && inputVValue.length() > 0 && outputCValue.length() > 0) {
    clearOutputCValue();
...
void handleCKey() {
  ...
  } else if (enteringSecondInput) {
    if (!inputVHasDecimal) {
      inputVValue += ".";
      inputVHasDecimal = true;
      lcd.setCursor(10 + inputVValue.length() - 1, 0);
      lcd.print(".");
    } else {
...
void resetCircuit() {
  ...
  sendData();
  Serial.println("ESP32 will reset in 5 seconds...");
  delay(3000);
  esp_restart();
}
...
void clearInputVValue() {
  inputVValue = "";
  lcd.setCursor(10, 0);  // Adjust cursor position if needed
  lcd.print("          ");
  lcd.setCursor(10, 0);
}
...
void runMeasureSendWaterVolume() {
  ...
  float pumpRate = 8;  // Pump rate in ml/s
  ...
  if (outputV > 0 && waterFilled == false) {
    int pumpTime = (outputV - 2) / pumpRate;  // Calculate the time in seconds to pump the desired volume
