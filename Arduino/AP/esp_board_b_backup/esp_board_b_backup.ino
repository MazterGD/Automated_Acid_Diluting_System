...
void loop() {
  ...
  // if (myData.BIsPressed && myData.ultrasonicObjectDetected && !myData.motorPartCompleted) {
  //   runMotorPart();
  // }
  if (waterFilled && !myData.motorPartCompleted) {
    // run motor part
...
void OnDataRecv(const esp_now_recv_info *recv_info, const uint8_t *incomingData, int len) {
  ...
  // if (myData.BIsPressed && !myData.motorPartCompleted) {
  //   runUltrasonicOutputCheck();
  // }
}
